      ...
      <div className={tokens.spacing.container}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course, index) => (
            <div 
              key={course.id}
              className={`group p-8 rounded-[40px] border-2 ${course.borderColor} ${course.bgColor} transition-all duration-500 hover:scale-[1.05] hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] hover:-translate-y-2 active:scale-95 flex flex-col justify-between h-full relative overflow-hidden cursor-pointer delay-[${index * 100}ms]`}
            >
              {/* Subtle pattern */}
              ...
              <button 
                className="w-full py-4 rounded-2xl border-2 font-bold text-sm flex items-center justify-center gap-2 transition-all hover:gap-4 mt-auto group-hover:bg-white group-hover:border-transparent group-hover:shadow-md"
                style={{ 
                  color: course.color, 
                  borderColor: course.borderColor + "40" // adding some transparency
                }}
              >
